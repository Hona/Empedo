...
namespace Empedo.Logging
{
    public static class Logger
    {
        ...
        public static void LogError(string message, Exception exception = null) =>
            LogToConsole(new LogMessage(LogType.Error, message, exception));

        public static void LogException(Exception exception, string message = null) =>
            LogToConsole(new LogMessage(LogType.Error, message ?? "No extra information.", exception));
        ...
        public static void LogToConsole(LogMessage logMessage)
        {
            ...
            LogTasks.Add(Task.Run(() =>
            {
                lock (_lock)
                {
                    ...
                    LogTasks = LogTasks.Where(x =>
                        !x.IsCanceled && !x.IsCompleted && !x.IsCompletedSuccessfully && !x.IsFaulted).ToList();
                }
            }));
        }
